...
let f_update_shader = function(){
    ...
    o_webgl_program = f_o_webgl_program(
        ...
        void main() {
            ...
            float na = atan(o_trn.x, o_trn.y)+3.14;
            fragColor = vec4(
                sin(n*18.6+iTime+sin(na*n_number))*.5+.5,
                sin(n*19.2+iTime+sin(na*n_number))*.5+.5,
                sin(n*19.8+iTime+sin(na*n_number))*.5+.5,
                sin(n*18.+iTime+na)*.5+.5
            );
        }
        `
    )
    ...
    o_state.o_ufloc__n_number = o_webgl_program?.o_ctx.getUniformLocation(o_webgl_program?.o_shader__program, 'n_number');
...
o_state = createNestedProxy(o_state, '', (path, oldValue, newValue) => {
    ...
    let o_el = document.querySelector(`[s_prop_sync="${path}"]`);
